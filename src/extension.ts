...
const state: { [key: string]: StateResponse['latestRelease'] } = {};
...
export async function activate(context: vscode.ExtensionContext) {
  vscode.commands.registerCommand('steroidTraces.viewInTextEditor', (params) =>
  ...
  view.onDidChangeSelection(async ({ selection: [item] }) => {
    ...
    for (const folder of openedFolders) {
      ...
      if (repo?.state.HEAD?.commit === item.commit) {
        ...
        const offset = getOffset(item.startLine - 1, item.startLine - 1, calculatedDiff) || 0;
        await vscode.window.showTextDocument(document, {
          selection: new vscode.Range(
            new vscode.Position(item.startLine - 1 + offset, item.startColumn - 1),
            new vscode.Position(item.startLine - 1 + offset, item.startColumn - 1),
          ),
        });
  ...
  const onDidChangeActiveTextEditor = vscode.window.onDidChangeActiveTextEditor(
    (textEditor) => textEditor && markCodePlaces(textEditor),
  );
...
async function markCodePlaces(textEditor: vscode.TextEditor) {
  ...
  const latestReleaseForRepo: Awaited<ReturnType<typeof getLatestRelease>> | undefined =
    state[commit];
...
function getOffset(
  trackingOriginalStartlLine: number,
  trackingOriginalEndLine: number,
  diffs: diff.Change[],
) {
  let originalIndex = 0;
  let modifiedIndex = 0;
